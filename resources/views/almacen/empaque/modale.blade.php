<div class="modal fade modal-slide-in-right" aria-hidden="true" role="dialog" tabindex="-1" id="modal-delete2-{{$materiales->id}}">
 ...
 <label class="col-sm-3 control-label">Medida <strog class="theme_color">*</strog></label>
  <div class="col-sm-8">
  <select name="medida{{$materiales->id}}"   class="form-control select"  data-live-search="true"   id="medida{{$materiales->id}}" >  
...
<script>
  function save($id2)
  {
    var select2 = document.getElementById('medida{{$materiales->id}}');
...
alert(document.getElementById('umedida{{$materiales->id}}').value);
alert(unidadaux);
